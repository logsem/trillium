...
Proof.
  ...
  rewrite map_fold_insert_L;
    [rewrite !map_fold_empty; set_solver|set_solver|by rewrite lookup_gset_to_gmap_None]. 
Qed.
...
Definition wp_group_proto `{!anerisPreG Σ Mdl} IPs A
  ...
  (∀ (aG : anerisG Mdl Σ), ⊢ |={⊤}=>
     ...
     observed_send_groups obs_send_sas -∗
     observed_receive_groups obs_rec_sas ={⊤}=∗
     WP (mkExpr ip e) @ s; (ip,0)%type; ⊤ {{v, ⌜φ v⌝ }}).
