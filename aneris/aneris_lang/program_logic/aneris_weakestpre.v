...
Definition aneris_wp_def `{!anerisG Mdl Σ} (ip : ip_address) (E : coPset)
  ...
  (∀ tid, is_node ip -∗
   wp NotStuck E (ip, tid)%type (mkExpr ip e) (λ v, ∃ w, ⌜v = mkVal ip w⌝ ∗ Φ w))%I.
...
Lemma aneris_wp_lift tid ip e E Φ :
  ...
  wp NotStuck E (ip,tid)%type (mkExpr ip e) (λ w, ∃ v, ⌜w = mkVal ip v⌝ ∗ Φ v).
...
(*     rewrite !wp_unfold /wp_pre /= /aneris_to_val /= He. simpl. *)
...
(*   iDestruct 1 as (δ2 ℓ) "((?&?&?&?&Hauth) & H & Hefs)". *)
(*   iExists δ2, ℓ. *)
(*   iFrame. *)
(* Qed. *)
...
(*           "(?&?&?&?&Hauth&?)". *)
(*   iSplit. *)
(*   { iDestruct ("Hwp" with "[//] [//] [//] [$]") as "[H _]". done. } *)
...
(*     iIntros (extr atr) "Hσ".  *)
(*     iDestruct "Hσ" as "(?&?&?&?&H'&?)". *)
(*     iDestruct (steps_lb_get with "H'") as "#H''". *)
...
Lemma aneris_wp_step_update_strong ip E1 E2 P e Φ :
  TCEq (to_val e) None → E2 ⊆ E1 →
  (|~{E1, E2}~> P) -∗ 
  WP e @[ip] E2 {{ v, P -∗ Φ v }} -∗
  WP e @[ip] E1 {{ v, Φ v}}.
Proof.
  rewrite !aneris_wp_unfold /aneris_wp_def.
  iIntros (He HE) "HP Hwp".
  iIntros (tid) "Hn". iDestruct ("Hwp" with "Hn") as "Hwp".
  iApply (wp_step_update_strong with "[$HP] [Hwp]"); [try done..|].
  { simpl. rewrite /aneris_to_val. rewrite He. done. }
...
Proof.
  ...
  iIntros (He HE) "HP Hwp".
  iIntros (tid) "Hn". iDestruct ("Hwp" with "Hn") as "Hwp".
  iApply (wp_step_update with "[$HP] [Hwp]"); [try done..|].
  { simpl. rewrite /aneris_to_val. rewrite He. done. }
  iApply (wp_wand with "Hwp").
  iIntros (v) "[%w [H HΦ]] HP". iDestruct ("HΦ" with "HP") as "HΦ".
  iFrame.
Qed.
Lemma aneris_wp_step_update_emp ip E1 e P Φ :
  TCEq (to_val e) None →
  (|~~> P) -∗
  WP e @[ip] E1 {{ v, P -∗ Φ v }} -∗
  WP e @[ip] E1 {{ Φ }}.
Proof.
  ...
  iIntros (He) "HP Hwp".
  iIntros (tid) "Hn". iDestruct ("Hwp" with "Hn") as "Hwp".
  iApply (wp_step_update_emp with "[$HP] [Hwp]"); [try done..|].
  { simpl. rewrite /aneris_to_val. rewrite He. done. }
  iApply (wp_wand with "Hwp").
  iIntros (v) "[%w [H HΦ]] HP". iDestruct ("HΦ" with "HP") as "HΦ".
  iFrame.
...
Proof.
  ...
  iIntros (He) "HP Hwp".
  iIntros (tid) "Hn". iDestruct ("Hwp" with "Hn") as "Hwp".
  iApply (wp_trp_update with "[$HP] [Hwp]"); [try done..|].
  { simpl. rewrite /aneris_to_val. rewrite He. done. }
  iApply (wp_wand with "Hwp").
  iIntros (v) "[%w [H HΦ]] HP". iDestruct ("HΦ" with "HP") as "HΦ".
  iFrame.
...
Proof.
  ...
  iIntros (He) "HP Hwp".
  iIntros (tid) "Hn". iDestruct ("Hwp" with "Hn") as "Hwp".
  iApply (wp_tr_use with "[$HP] [Hwp]"); [try done..|].
  { simpl. rewrite /aneris_to_val. rewrite He. done. }
  iApply (wp_wand with "Hwp").
  iIntros (v) "[%w [H HΦ]] HP H£". iDestruct ("HΦ" with "HP H£") as "HΦ".
  iFrame.
Qed.
...
Proof.
  ...
  iIntros "(Hev & Hσ & H)".
  iMod (aneris_state_interp_socket_interp_allocate_singleton_groups with "Hσ Hsag")
    as "[Hσ HΨ]".
...
Proof.
  ...
  iIntros (extr atr K tp1 tp2 σ1 Hextr Hlocale Htr).
  iIntros "(Hev & Hσ & H)".
  iMod (aneris_state_interp_socket_interp_allocate_fun_groups with "Hσ Hsag") as "[Hσ HΨ]".
  iDestruct ("Hwp" with "HΨ Hin") as "Hwp".
  rewrite !wp_unfold /wp_def /wp_pre. simpl. rewrite /aneris_to_val He.
  iApply ("Hwp" with "[//] [//] [//] [$Hev $Hσ $H]").
Qed.
...
Proof.
  ...
  iIntros (extr atr K tp1 tp2 σ1 Hextr Hlocale Htr).
  iIntros "(Hev & Hσ & H)".
  iMod (aneris_state_interp_socket_interp_allocate_groups with "Hσ Hsag") as "[Hσ HΨ]".
  iDestruct ("Hwp" with "HΨ Hin") as "Hwp".
  rewrite !wp_unfold /wp_def /wp_pre. simpl. rewrite /aneris_to_val He.
  iApply ("Hwp" with "[//] [//] [//] [$Hev $Hσ $H]").
Qed.
...
Section proofmode_classes.
  ...
  Global Instance elim_modal_step_upd_wp_1 ip E1 E2 P e Φ :
    TCEq (to_val e) None →
    ElimModal (E2 ⊆ E1) false false (|~{E1, E2}~> P) emp (WP e @[ip] E1 {{ Φ }}) (WP e @[ip] E2 {{ v, P -∗ Φ v }})%I.
  Proof.
    rewrite /ElimModal /=. iIntros (??) "[HP HPQ]".
    by iApply (aneris_wp_step_update_strong with "[$] (HPQ [//])").
  Qed.
  Global Instance elim_modal_step_upd_step_upd_2 ip E1 E2 P e Φ :
    TCEq (to_val e) None →
    ElimModal (E1 ⊆ E2) false false (|~{E1}~> P) emp (WP e @[ip] E2 {{ Φ }}) (WP e @[ip] E2∖E1 {{ v, P -∗ Φ v }})%I.
  Proof.
  ...
  Qed.
  Global Instance elim_modal_step_upd_step_upd_3 ip E P e Φ :
    TCEq (to_val e) None →
    ElimModal True false false (|~~> P) emp (WP e @[ip] E {{ Φ }}) (WP e @[ip] E {{ v, P -∗ Φ v }})%I.
  Proof.
    rewrite /ElimModal /=. iIntros (? Hle) "[Hupd HPQ]".
