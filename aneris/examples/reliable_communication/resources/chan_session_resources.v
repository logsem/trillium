...
Section OneShot.
  ...
  Proof. iApply own_update. apply SM_update. Qed.
...
Section iProto_sessions.
  ...
  Definition can_init
    (γs : session_name) (clt_addr : socket_address) (p : iProto Σ val) (s : side) : iProp Σ :=
  ...
  Lemma session_map_update
        (M : session_names_map) (sa : socket_address) (p : iProto Σ val)
    ...
    Proof.
    iIntros (Hfresh) "Hkn".
    iMod (step_get_Ses_init (N.@ (socket_address_to_str sa)) _ p)
      as (γ_s) "(Hownl & Hownr)".
