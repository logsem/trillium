...
Section Proof_of_server_conn_step_3.
  ...
  Proof.
    ...
    { iDestruct "HlRes" as (ackid) "[%Hreq [%n [-> [%γ [Htok' Hsidx']]]]]".
      ...
      iSplit; last first.
      { iDestruct "Hopened" as "[H1 $]". } 
      iLeft. iExists _. iSplit; [naive_solver|]. iExists _.
      iSplit; [done|]. subst. done. }
    ...
    { iExists ({[m]} ∪ R0), T0, cmv.
      ...
      iApply "Hmono_smr".
      iApply "HknResAcc".
      iExists _, ck1. iFrame "Hopened HCkF".
      iSplitR.
      { iFrame "#". done. }
      iRight.
      iExists γc, _, _, _, _, _. iFrame "HsidLB Hack".
    ...
    { apply bool_decide_eq_true_1 in HmR0.
      ...
      - iDestruct "HlRes" as (s i ->) "Hinit".
        ...
        { wp_pures.
          ...
          { do 2 (iSplit; first done).
            iApply "HknResAcc".
            iExists _, ck0. iFrame "Hopened HCkF".
            iSplitR.
            { iFrame "#". done. }
            iRight.
            iExists _, _, _, _, _, _.
