...
Section Proof_of_send.
  ...
  Proof.
    ...
    iIntros (rv Hq').
    wp_bind (Store _ _).
    iApply (aneris_wp_step_update _ _ (⊤ ∖ ↑((chan_N (endpoint_chan_name γe)))) with "[Hp]"); [done| |].
    { replace (⊤ ∖ (⊤ ∖ ↑((chan_N (endpoint_chan_name γe))))) with ((↑((chan_N (endpoint_chan_name γe)))):coPset); last first.
      { rewrite difference_difference_r_L. set_solver. }
      iApply (step_update_send with "Hp"). by rewrite iMsg_base_eq. }
    ...
    iMod (mono_nat_own_update (sidLB + length vs + 1) with "[Hsidx Hsidx']")
      as "[Hsidx _]";[|iApply fractional_half; iFrame|]; [lia|].
