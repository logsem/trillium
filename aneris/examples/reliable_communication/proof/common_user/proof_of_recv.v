...
Section Proof_of_recv.
  ...
  Proof.
    ...
    iMod (mono_nat_own_update (ridx + 1) with "[Hridx Hridx']")
      as "[Hridx _]";[|iApply fractional_half; iFrame|]; [lia|].
    iDestruct "Hridx" as "[Hridx Hridx']".
    replace (ridx + 0) with ridx by lia.
    iApply (aneris_wp_step_update _ _ (⊤ ∖ ↑((chan_N (endpoint_chan_name γe))))
                with "[Hp Hfrag]");
      [done| |].
    { replace (⊤ ∖ (⊤ ∖ ↑((chan_N (endpoint_chan_name γe))))) with ((↑((chan_N (endpoint_chan_name γe)))):coPset); last first.
      ...
      by iApply (step_update_recv with "Hp Hfrag"). }
    wp_store.
