From trillium.traces Require Import my_omega trace_len inftraces trace_utils utils_logic.
...
Section After.
  ...
  Proof. 
    ...
    2: { split; [by intros (?&?&?)| ].
         pose proof (trace_has_len tr) as [len LEN].
         intros NTH.
         opose proof (proj1 (trace_lookup_dom_strong _ _ LEN n) _); eauto.
         eapply trace_len_neg in AFTER; eauto. lia_NO len. }
  ...
  Proof.
    pose proof (trace_has_len tr) as [len LEN]. 
    opose proof (proj2 (trace_lookup_dom_strong _ _ LEN i) _) as X.
    { eapply trace_lookup_dom; eauto.
      by rewrite Nat.add_1_r. }
    destruct X as (?&?&st'&ITH).
    enough (st' = st2).
    { subst. eauto. }    
...
Section TracesMatch.
  ...
  Proof. 
    ...
    opose proof (traces_match_same_length _ _ _ _ tr1 tr2) as X.
    ospecialize * X; eauto. subst x. 
    ...
    opose proof (proj1 (trace_state_lookup_simpl' tr1 n s1) _) as ST1; eauto.  
    opose proof (proj1 (trace_state_lookup_simpl' tr2 n s2) _) as ST2; eauto.  
    simpl in *.
    pose proof (proj2 (state_lookup_after' _ _ _) ST1) as (atr1 & AFTER1 & A1).
    pose proof @traces_match_after' as X.
    specialize X with (tr1 := tr1) (tr2 := tr2); eauto. ospecialize * X; eauto.
    destruct X as (atr2 & AFTER2 & A2). 
    split.
    ...
    4: done.
    3: { opose proof (proj1 (trace_lookup_dom_strong _ _ LEN2 n) _); eauto.
         opose proof (proj1 (trace_lookup_dom_eq _ _ LEN1 n) _); eauto.
         subst. simpl in *. lia. }
    2: { opose proof (proj1 (trace_lookup_dom_strong _ _ LEN1 n) _); eauto.
         opose proof (proj1 (trace_lookup_dom_eq _ _ LEN2 n) _); eauto.
         subst. simpl in *. lia. }
...
Section UptoStutter.
  ...
  Proof.
    ...
    opose proof (upto_stutter_after' _ _ n UPTO _) as X; eauto.
    destruct X as (n' & str' & AFTER' & UPTOn).
    exists n'.
  ...
  Proof.
    ...
    opose proof (upto_stutter_after _ _ n' UPTO _) as X; eauto.
    destruct X as (n & btr' & AFTER' & UPTOn).
    exists n.
