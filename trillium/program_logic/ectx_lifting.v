...
Lemma wp_lift_head_step_fupd {s f E Φ} e1 ζ:
  to_val e1 = None →
...
Lemma wp_lift_head_step {s f E Φ} e1 ζ:
  to_val e1 = None →
  (∀ (extr : execution_trace Λ) (atr : auxiliary_trace M) K tp1 tp2 σ1,
    ⌜valid_exec extr⌝ -∗
    ⌜trace_ends_in extr (tp1 ++ fill K e1 :: tp2, σ1)⌝ →
...
Lemma wp_lift_head_stuck f E Φ e ζ:
  to_val e = None →
...
Lemma wp_lift_pure_head_stuck f ζ E Φ e :
  to_val e = None →
  sub_redexes_are_values e →
  (∀ σ, head_stuck e σ) →
  ⊢ WP e @ f; MaybeStuck; ζ; E {{ Φ }}.
...
Lemma wp_lift_atomic_head_step_fupd {s f E1 E2 Φ} e1 ζ:
  to_val e1 = None →
  (∀ (extr : execution_trace Λ) (atr : auxiliary_trace M) K tp1 tp2 σ1,
    ⌜valid_exec extr⌝ -∗
    ⌜trace_ends_in extr (tp1 ++ fill K e1 :: tp2, σ1)⌝ →
    ...
    ∀ e2 σ2 efs, ⌜head_step e1 σ1 e2 σ2 efs⌝ ={E1}[E2]▷=∗
      ...
      ([∗ list] i ↦ef ∈ efs, WP ef @ f; s; locale_of (tp1 ++ ectx_fill K e1 :: tp2 ++ (take i efs)) ef; ⊤
         {{ fork_post (locale_of (tp1 ++ ectx_fill K e1 :: tp2 ++ (take i efs)) ef) }}) ∗ ⌜ f = CannotFork -> efs = [] ⌝)
  ⊢ WP e1 @ f; s; ζ; E1 {{ Φ }}.
Proof.
...
Lemma wp_lift_atomic_head_step {s f E Φ} e1 ζ:
  to_val e1 = None →
  (∀ (extr : execution_trace Λ) (atr : auxiliary_trace M) K tp1 tp2 σ1,
    ⌜valid_exec extr⌝ -∗
    ⌜trace_ends_in extr (tp1 ++ fill K e1 :: tp2, σ1)⌝ →
...
Lemma wp_lift_atomic_head_step_no_fork_fupd {s f E1 E2 Φ} e1 ζ:
  to_val e1 = None →
  (∀ (extr : execution_trace Λ) (atr : auxiliary_trace M) K tp1 tp2 σ1,
    ⌜valid_exec extr⌝ -∗
    ⌜trace_ends_in extr (tp1 ++ fill K e1 :: tp2, σ1)⌝ →
...
Lemma wp_lift_atomic_head_step_no_fork {s f E Φ} e1 ζ:
  to_val e1 = None →
  (∀ (extr : execution_trace Λ) (atr : auxiliary_trace M) K tp1 tp2 σ1,
    ⌜valid_exec extr⌝ -∗
    ⌜trace_ends_in extr (tp1 ++ fill K e1 :: tp2, σ1)⌝ →
...
Lemma wp_lift_pure_det_head_step_no_fork
      `{!AllowsPureStep M Σ}  {s f E E' Φ} e1 e2 ζ:
  to_val e1 = None →
...
Lemma wp_lift_pure_det_head_step_no_fork'
      `{!AllowsPureStep M Σ} {s f E Φ} e1 e2 ζ:
  to_val e1 = None →
  (∀ σ1, head_reducible e1 σ1) →
  (∀ σ1 e2' σ2 efs',
    head_step e1 σ1 e2' σ2 efs' → σ2 = σ1 ∧ e2' = e2 ∧ efs' = []) →
  ▷ WP e2 @ f; s; ζ; E {{ Φ }} ⊢ WP e1 @ f; s; ζ; E {{ Φ }}.
Proof using Hinh.
  intros. rewrite -[(WP e1 @ f; s; _; _ {{ _ }})%I]wp_lift_pure_det_head_step_no_fork //.
