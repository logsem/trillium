...
Section ectx_language_mixin.
  ...
  Record EctxLanguageMixin := {
    ...
    mixin_val_head_stuck e1 σ1 α e2 σ2 efs :
      head_step e1 σ1 α e2 σ2 efs → to_val e1 = None;
    ...
    (** Given a head redex [e1_redex] somewhere in a term, and another
        ...
        [head_redex_unique]. *)
    mixin_step_by_val K' K_redex e1' e1_redex σ1 α  e2 σ2 efs :
      fill K' e1' = fill K_redex e1_redex →
      to_val e1' = None →
      head_step e1_redex σ1 α e2 σ2 efs →
    ...
    mixin_head_ctx_step_val K e σ1 α e2 σ2 efs :
      head_step (fill K e) σ1 α e2 σ2 efs → is_Some (to_val e) ∨ K = empty_ectx;
...
Arguments EctxLanguage {_ _ _ _ _ _ _ _ _ _ _ _} _ _ _.
...
Section ectx_language.
  ...
  Lemma val_head_stuck e1 σ1 α e2 σ2 efs : head_step e1 σ1 α e2 σ2 efs → to_val e1 = None.
  ...
  Lemma fill_positive K K' e e' :
    ...
    (∃ K'', K' = comp_ectx K K'') ∨ (∃ K'', K = comp_ectx K' K'').
  Proof. apply ectx_language_mixin. Qed.
  Lemma step_by_val K' K_redex e1' e1_redex σ1 α e2 σ2 efs :
      fill K' e1' = fill K_redex e1_redex →
      to_val e1' = None →
      head_step e1_redex σ1 α e2 σ2 efs →
      ∃ K'', K_redex = comp_ectx K' K''.
  Proof. apply ectx_language_mixin. Qed.
  Lemma head_ctx_step_val K e σ1 α e2 σ2 efs :
    head_step (fill K e) σ1 α e2 σ2 efs → is_Some (to_val e) ∨ K = empty_ectx.
  Proof. apply ectx_language_mixin. Qed.

  Definition head_reducible (e : expr Λ) (σ : state Λ) :=
    ∃ α e' σ' efs, head_step e σ α e' σ' efs.
  ...
  Inductive prim_step (e1 : expr Λ) (σ1 : state Λ) (α : action Λ) 
    ...
    Ectx_step K e1' e2' :
      ...
      head_step e1' σ1 α e2' σ2 efs → prim_step e1 σ1 α e2 σ2 efs.

  Lemma Ectx_step' K e1 σ1 α e2 σ2 efs :
    head_step e1 σ1 α e2 σ2 efs → prim_step (fill K e1) σ1 α (fill K e2) σ2 efs.
  ...
  Proof.
    ...
    - intros K; split_and!.
      ...
      + intros e1 σ1 α e2 σ2 efs Hnval [K'' e1'' e2'' Heq1 -> Hstep].
        destruct (step_by_val K K'' e1 e1'' σ1 α e2'' σ2 efs) as [K' ->]; eauto.
    ...
    - apply fill_inj.
    - apply fill_positive.
    - intros e1 e2 α t1 σ1 σ2 efs Hpstep. inversion Hpstep; simplify_eq.
  ...
  Proof.
    intros Heq (α & e2 & σ2 & efs & Hred) (α' & e2' & σ2' & efs' & Hred').
  ...
  Lemma head_prim_step e1 σ1 α e2 σ2 efs :
    head_step e1 σ1 α e2 σ2 efs → prim_step e1 σ1 α e2 σ2 efs.
  Proof. apply Ectx_step with empty_ectx; by rewrite ?fill_empty. Qed.

  Lemma head_step_not_stuck e σ α e' σ' efs : head_step e σ α e' σ' efs → not_stuck e σ.
  Proof. rewrite /not_stuck /reducible /=. eauto 10 using head_prim_step. Qed.

  Lemma fill_prim_step K e1 σ1 α e2 σ2 efs :
    prim_step e1 σ1 α e2 σ2 efs → prim_step (fill K e1) σ1 α (fill K e2) σ2 efs.
  Proof.
  ...
  Proof.
    intros (α&e'&σ'&efs&?). exists α, (fill K e'), σ', efs.
    by apply fill_prim_step.
  Qed.
  Lemma head_prim_reducible e σ : head_reducible e σ → reducible e σ.
  Proof. intros (α&e'&σ'&efs&?). eexists α, e', σ', efs. by apply head_prim_step. Qed.
  ...
  Proof.
    intros (α&e'&σ'&efs&[K e1' e2' -> -> Hstep]) ?.
  ...
  Proof.
    intros Hatomic_step Hatomic_fill σ α e' σ' efs [K e1' e2' -> -> Hstep].
    assert (K = empty_ectx) as -> by eauto 10 using val_head_stuck.
  ...
  Lemma head_reducible_prim_step_ctx K e1 σ1 α e2 σ2 efs :
    head_reducible e1 σ1 →
    prim_step (fill K e1) σ1 α e2 σ2 efs →
    ∃ e2', e2 = fill K e2' ∧ head_step e1 σ1 α e2' σ2 efs.
  Proof.
    intros (α''&e2''&σ2''&efs''&HhstepK) [K' e1' e2' HKe1 -> Hstep].
  ...
  Lemma head_reducible_prim_step e1 σ1 α e2 σ2 efs :
    head_reducible e1 σ1 →
  ...
  Proof.
    ...
    - intros σ. destruct (Hp1 σ) as (α & e2' & σ2 & efs & ?).
      eexists α, e2', σ2, efs. by apply head_prim_step.
    - intros σ1 α e2' σ2 efs ?%head_reducible_prim_step; eauto.
  Qed.
  ...
  Lemma head_locale_step K e1 e2 α tp1 tp2 efs σ1 σ2 :
    head_step e1 σ1 α e2 σ2 efs →
    locale_step
      (tp1 ++ fill K e1 :: tp2, σ1)
      (inl (locale_of tp1 e1, α))
...
Definition LanguageOfEctx (Λ : ectxLanguage) : language :=
  let '@EctxLanguage E V C St Loc locale action of_val to_val empty comp fill head config loc_of (* config_enabled *) mix := Λ in
  @Language E V C St Loc locale action of_val to_val _ config loc_of (* config_enabled  *) comp empty fill
    (@ectx_lang_mixin
       (@EctxLanguage E V C St Loc locale action of_val to_val empty comp fill head config loc_of (* config_enabled *) mix)).
