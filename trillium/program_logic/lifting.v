...
Lemma wp_lift_step_fupdN s E Φ e1 ζ:
  ...
  (∀ (extr : execution_trace Λ) (atr : auxiliary_trace M) K tp1 tp2 σ1,
    ...
    ∀ α e2 σ2 efs, ⌜prim_step e1 σ1 α e2 σ2 efs⌝ ={∅}▷=∗^(S $ trace_length extr) |={∅,E}=>
     ∃ δ2 ℓ,
       state_interp
...
Lemma wp_lift_step_fupd s E Φ e1 ζ:
  ...
  (∀ (extr : execution_trace Λ) (atr : auxiliary_trace M) K tp1 tp2 σ1,
    ...
    state_interp extr atr ={E,∅}=∗
    ⌜if s is NotStuck then reducible e1 σ1 else True⌝ ∗
    ∀ α e2 σ2 efs, ⌜prim_step e1 σ1 α e2 σ2 efs⌝ ={∅}=∗ ▷ |={∅,E}=>
     ∃ δ2 ℓ,
       state_interp
         (trace_extend extr (inl (ζ,α)) (tp1 ++ ectx_fill K e2 :: tp2 ++ efs, σ2))
         (trace_extend atr ℓ δ2) ∗
  ...
  ⊢ WP e1 @ s; ζ; E {{ Φ }}.
Proof.
    intros ?. rewrite -wp_lift_step_fupdN; [|done]. simpl. do 28 f_equiv.
...
Proof.
  ...
  iIntros (α e2 σ2 efs ?). by case: (Hirr α e2 σ2 efs).
 Qed.
...
Lemma wp_lift_atomic_step_fupd {s E1 E2 Φ} e1 ζ:
  ...
  (∀ (extr : execution_trace Λ) (atr : auxiliary_trace M) K tp1 tp2 σ1,
    ...
    ⌜if s is NotStuck then reducible e1 σ1 else True⌝ ∗
    ∀ α e2 σ2 efs, ⌜prim_step e1 σ1 α e2 σ2 efs⌝ ={E1}[E2]▷=∗
      ∃ δ2 ℓ, 
        state_interp
          (trace_extend extr (inl (ζ,α)) (tp1 ++ ectx_fill K e2 :: tp2 ++ efs, σ2))
          (trace_extend atr ℓ δ2) ∗
...
Proof.
  ...
  iIntros "!>" (α e2 σ2 efs ?). iMod "Hclose" as "_".
  iMod ("H" $! α e2 σ2 efs with "[#]") as "H"; [done|].
