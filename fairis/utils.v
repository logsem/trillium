...
Proof.
  intros n Pn.
  opose proof * (min_prop_dec_impl (P ∘ Pos.of_nat) _ (Pos.to_nat n)) as [m [Pm MINm]].
  { red. by rewrite Pos2Nat.id. }
  exists (Pos.of_nat m). split; auto.
  intros.
  ospecialize * (MINm (Pos.to_nat k) _). 
  { red. by rewrite Pos2Nat.id. }
  lia. 
Qed.
...
Proof using.
  red. intros. opose proof * min_prop_dec_impl as [??]; eauto.
Qed. 
