...
Section model_state_lemmas.
  ...
  Program Definition model_update_locale (δ : LM) (ζ : locale Λ) : LM :=      
    {|
      ls_under := δ.(ls_under);
      ls_fuel :=
        map_imap (λ ρ f, if bool_decide (δ.(ls_mapping) !! ρ = Some ζ)
                    then Some $ f - 1
                    else Some f)
        (filter (λ ρf, (ρf.1 ∈ M.(live_roles) δ.(ls_under) ∨
                         δ.(ls_mapping) !! ρf.1 ≠ Some ζ)) δ.(ls_fuel));
      ls_mapping :=
        filter (λ ρζ, (ρζ.1 ∈ M.(live_roles) δ.(ls_under) ∨ ζ ≠ ρζ.2))
               δ.(ls_mapping);
      |}.
  ...
  Definition model_can_step (δ : LM) (ζ : locale Λ) : Prop :=
    (∃ ρ : Role, ls_mapping δ !! ρ = Some ζ) ∧
    (∀ ρ, δ.(ls_mapping) !! ρ = Some ζ →
          ρ ∈ M.(live_roles) δ.(ls_under) →
          ∃ n, δ.(ls_fuel) !! ρ = Some (S n)).

  Lemma model_update_locale_spec extr (auxtr : auxiliary_trace LM) ζ c2 :
    model_can_step (trace_last auxtr) ζ →
    (* This can probably be relaxed *)
    tids_smaller c2.1 (model_update_locale (trace_last auxtr) ζ) →
    valid_state_evolution_fairness
      (extr :tr[Some ζ]: c2)
      (auxtr :tr[Silent_step ζ]: (model_update_locale (trace_last auxtr) ζ)).
  Proof.
  Admitted.

  Lemma model_state_interp_can_step es δ ζ fs :
    model_state_interp es δ -∗ has_fuels_S ζ fs -∗ ⌜model_can_step δ ζ⌝.
  Proof. Admitted.
  ...
  Lemma model_state_interp_update c1 c2 δ ζ fs :
    ...
    has_fuels_S ζ fs ==∗
    model_state_interp c2.1 (model_update_locale δ ζ) ∗
    has_fuels ζ fs.
  Proof.
    iIntros (Hdom Hstep) "Hm Hfuel".
