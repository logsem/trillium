...
From trillium.fairness Require Export fairness fair_termination fairness_finiteness fuel fuel_termination map_included_utils resources action_model.
From trillium.program_logic Require Import ectx_lifting.
From trillium.fairness.heap_lang Require Import lang locales_lemmas iris_inst.
...
Section adequacy.
  ...
  Proof using hpreGS.
    ...
    iSpecialize ("Hwp" with "Hσ Hmodf1 Hfr [Hmapf]").
  ...
  Theorem simulation_adequacy (s: stuckness) (e1 : expr) σ1 (s1: M) (FR: gset _):
  ...
  Proof using hpreGS.
    ...
    rewrite Heq.
    apply (strong_simulation_adequacy s _ _ _ FR) =>//.
    { rewrite -Heq. done. }
    iIntros (Hinv) "". simpl. 
  ...
  Theorem simulation_adequacy_inftraces (s: stuckness) FR
    e1 σ1 (s1: M)
  ...
  Proof using hpreGS.
    ...
    Unshelve.
    - econstructor.
    - apply (simulation_adequacy s _ _ _ FR) => //.
    - done.
  Qed.

  Definition heap_lang_extrace : Type := extrace heap_lang.

  Theorem simulation_adequacy_traces (s: stuckness) FR
        e1 (s1: M)
  ...
  Proof using hpreGS.
    ...
    have [iatr Hbig] : exists iatr,
        @valid_inf_system_trace
          ...
          (from_trace extr)
          iatr.
    { apply (simulation_adequacy_inftraces s FR); eauto.
  ...
  Theorem simulation_adequacy_model_trace
    (s: stuckness) FR
    e1 (s1: M)
    (extr : heap_lang_extrace)
    (Hvex : extrace_valid extr)
  ...
  Proof using hpreGS.
    ...
    destruct (simulation_adequacy_traces
                _ FR e1 s1 extr Hvex Hexfirst Hfb Hlr Hwp) as [auxtr Hmatch].
...
Theorem simulation_adequacy_terminate
  (s: stuckness)
        e1 s1 FR
        (extr : heap_lang_extrace)
        (Hexfirst : (trfirst extr).1 = [e1])
...
Proof using hpreGS.
  ...
  destruct (simulation_adequacy_model_trace
              _ FR e1 s1 extr Hvex Hexfirst Hfb Hlr Hwp) as (auxtr&mtr&Hmatch&Hupto).
...
Theorem simulation_adequacy_terminate_ftm `{FairTerminatingModel M}
        s e1 (s1: M) FR
        (extr : heap_lang_extrace)
        (Hexfirst : (trfirst extr).1 = [e1])
  :
