...
From trillium.fairness.heap_lang.examples.even_odd Require Import eo_vs_mod.
...
#[local] Instance proof_irrel_trans s x:
  ProofIrrel ((let '(s', ℓ) := x in fmtrans the_fair_model s ℓ s'): Prop).
Proof. apply make_proof_irrel. Qed.

Lemma model_finitary s:
  Finite { '(s', ℓ) | fmtrans the_fair_model s ℓ s'}.
Proof.
  destruct prod_AM_fin_branch' as [ns FIN].
  eapply (in_list_finite ((fun '(x, y, z) => (x, z)) <$> ns s)).
  intros [??] STEP. eapply elem_of_list_fmap.
  inversion STEP; subst; simpl in *.
  all: eexists; split; [| apply FIN]; eauto.
Qed. 
...
Definition evenodd_mdl_progress (tr : evenodd_mtrace) :=
  ∀ i, ∃ n, pred_at tr n (λ s _, s = (i, i)).

Definition evenodd_mdl_mono (tr : evenodd_mtrace) :=
  ∀ n, ∃ i, pred_at tr n (λ s _, s = (i, i)) ∧
            pred_at tr (S n) (λ s _, ∃ j, s = (j, j) ∧ i ≤ j).
