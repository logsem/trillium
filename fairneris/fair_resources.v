...
Class LiveModelEq `{GoodLang Λ} `{N: EnvModel Λ}
  `(LM: !LiveModel Λ (joint_model Mod N)) := {
    cfg_labels_match_is_eq: ∀ x y, lm_cfg_labels_match LM x y ↔ x = y;
    actions_match_is_eq: ∀ x y, lm_actions_match LM x y ↔ x = y;
}.
Arguments LiveModelEq {_ _ _ _ _ _ _ _ _ _ _} _.

Definition live_model_of_user `{GoodLang Λ} (M: UserModel Λ) N : LiveModel Λ (joint_model M N) :=
 {|
    lm_cfg_labels_match (x : config_label Λ) (y : fmconfig (joint_model M N)) := x = y;
    lm_actions_match x y := x = y;
 |}.

Instance live_model_of_user_eq `{GoodLang Λ} (M: UserModel Λ) N : LiveModelEq (live_model_of_user M N).
...
Global Arguments fairnessGS {_ _ _ _ _ _ _ _ _ _ _} LM Σ.
Global Arguments fairness_model_name {_ _ _ _ _ _ _ _ _ _ _ LM Σ} _.
Global Arguments fairness_model_fuel_mapping_name {Λ _ _ _ _ _ _ _ _ M Net LM Σ} _ : assert.
Global Arguments fairness_model_free_roles_name {Λ _ _ _ _ _ _ _ _ M Net LM Σ} _ : assert.
...
Section model_state_lemmas.
  ...
  Proof.
    ...
    iDestruct "Hm" as "(%fm&[%Hfmle%Hdom]&%Hfmdead&%Htp&Hm&Hfm&%Hsm&%Hcoh)".
    iDestruct (has_fuels_agree with "Hfm Hfs") as %Hagree.
    iMod (has_fuels_decr with "Hfm Hfs") as "[Hfm $]".
    iModIntro. iExists _. iFrame. iPureIntro.
    split; [|split; [|split; [|split;try done]]].
    ...
    - eapply env_state_coh_preserved=>//.
  Qed.
  ...
  Lemma model_interp_state_coh c (δ : LiveState _ (joint_model M Net)) :
    model_state_interp c δ -∗ ⌜ env_state_coh (N := Net) c.2 ⌝.
  Proof.
    by iIntros "(%fm&[%Hfmle%Hdom]&%Hfmdead&%Htp&Hm&Hfm&%Hsm&%Hcoh)"; iPureIntro.
  Qed.
  ...
  Proof.
    ...
    iDestruct (model_interp_state_coh _ _ with "Hm") as %?.
  ...
  Proof.
    ...
    iDestruct "Hm" as "(%fm&%Hfmle&%Hfmdead&%Htp&Hm&Hfm&%Hsm1&%Hcoh)".
  ...
  Lemma update_model_step `{!LiveModelEq LM}
    ...
    env_states_match c2 s2.2  →
    fmtrans _ s1 (inl (ρ, act)) s2 →
    env_state δ1 = s1.2 →
    ...
    ∃ (δ2: LM),
      ⌜valid_state_evolution_fairness
        (extr :tr[inl (ζ, act)]: c2) (auxtr :tr[Take_step ρ act ζ act]: δ2)⌝ ∗
      has_fuels ζ ({[ρ := fm_fl s2]} ∪ fs) ∗
      frag_model_is s2.1 ∗ model_state_interp c2 δ2.
  Proof.
    iIntros (Hlive Hdom Hlast Hstep Hcfg Htrans Henv) "Hfuel Hfrag %Hvse Hm".
    ...
    iFrame "Hm Hf Hfrag".
    iPureIntro. subst.
    eapply model_update_locale_spec_model_step=>//.
  Qed.
  ...
  - intros ρ' Hin.
    ...
    - simpl. eapply env_match_internal_step=>//.
