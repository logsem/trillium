...
From fairneris Require Import fairness fuel map_included_utils.
...
Section model_state_interp.
  ...
  Definition config_update_state : iProp Σ :=
    ∀ m1 m2 cl fl, ⌜ LM.(lm_cfg_action) m1 cl = (fl, m2) ⌝ -∗ auth_model_is m1 ={⊤}=∗ auth_model_is m2.

  Definition model_state_interp (tp: list $ expr Λ) (δ: LiveStateData Λ M): iProp Σ :=
    □ config_update_state ∗
    ∃ fuel_map,
...
Section model_state_lemmas.
  ...
  Proof.
    ...
    iSplit. iFrame. iExists _. iFrame.
  ...
  Proof.
    ...
    iModIntro.
    iFrame "Hst". iFrame "Hfs". iSplit; first iFrame.
  ...
  (* TODO: Move this *)
  Lemma silent_step_suff_data_weak act fl `(δ: LiveState Λ M)
        (fs fs' : gmap _ nat) ζ:
    δ.(ls_map) !! ζ = Some fs →
    fs ≠ ∅ →
    map_included (<) fs' fs →
  ...
  Lemma silent_step_suff_data_weak_alt act fl (δ δ' : LiveState Λ M)
        (fs fs' : gmap _ nat) ζ :
  ...
  Lemma model_can_fuel_step_trans act fl ζ (δ δ' : LiveState Λ M) :
  ...
  Lemma fuel_map_preserve_threadpool_fuel_step
        act c1 ζ c2 (fm1 fm2 : gmap _ (gmap (fmrole M) nat)) :
  ...
  Proof.
    ...
    iModIntro. iSplit; first iFrame. iExists _. iFrame. iPureIntro.
    split; [|split].
  ...
  Lemma model_step_suff_data_weak_alt (δ1 δ2 : LiveState Λ M) ρ fmact act
        (fs fs': gmap _ nat) ζ :
  ...
  Definition model_can_model_step (δ1 : LM) (ζ : locale Λ) (ρ : fmrole M) (δ2 : LM) fmact : Prop :=
  ...
  Lemma model_can_model_step_trans ζ ρ (δ δ' : LiveState Λ M) fmact act:
    model_can_model_step δ ζ ρ δ' fmact → ls_trans (LM.(lm_fl)) δ (Take_step ρ fmact ζ act) δ'.
  Proof.
    destruct 1 as (?&?&?&?&?&?&?&?&?&?).
  ...
  Lemma model_update_locale_spec_model_step extr
        (auxtr : auxiliary_trace LM) ζ c2 ρs ρ δ2 s2 fmact act :
  ...
  Lemma update_model_step
        ...
        (auxtr: auxiliary_trace LM) c2 (s1 s2 : M) fs ρ (δ1 : LM) ζ f fmact act :
    M.(live_roles) s2 ⊆ M.(live_roles) s1 →
    ρ ∉ dom fs →
    trace_last auxtr = δ1 →
    locale_step (trace_last extr) (inl (ζ, act)) c2 →
  ...
  Proof.
    iIntros (Hlive Hdom Hlast Hstep Htrans) "Hfuel Hfrag %Hvse Hm".
  ...
  Lemma silent_step_suff_data_fork_weak act fl `(δ: LiveState Λ M)
  ...
  (* TODO: Change original lemma to not existentially quantify new state *)
  Lemma silent_step_suff_data_fork_weak_alt act fl (δ δ': LiveState Λ M)
        (fs fs1 fs2 : gmap _ nat) ζ ζf :
    δ.(ls_under) = δ'.(ls_under) →
    δ.(ls_map) !! ζ = Some fs →
  ...
  Lemma model_can_fork_step_trans act fl ζ ζf (δ δ' : LiveState Λ M) :
