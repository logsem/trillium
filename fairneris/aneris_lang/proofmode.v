...
From trillium.program_logic Require Import atomic weakestpre.
From fairneris.aneris_lang Require Export tactics lifting network_model aneris_lang resources.
...
Proof.
  ...
  { inversion HPE. rewrite has_fuel_fuels_plus_0. simplify_eq. simpl.
    iIntros "A B". iApply "A". iAssumption. }
...
Lemma maps_gt_n {Mdl} (fs: gmap (usr_role Mdl) _) n:
...
Lemma tac_mu_fuel
  `{LM : LiveModel aneris_lang (joint_model M net_model)} `{!LiveModelEq LM} `{!anerisG LM Σ}
  Δ ζ E i fs P :
  envs_lookup i Δ = Some (false, has_fuels ζ fs)%I →
  fs ≠ ∅ ->
  (∀ (ρ : usr_role M) (f : nat), fs !! ρ = Some f → (f ≥ 1)%nat) ->
...
Proof.
  ...
  iApply (mu_step_fuel _ _ ((λ m : nat, m - 1) <$> fs) with "[H1]") =>//.
  - by intros ?%fmap_empty_inv.
  - iNext. rewrite has_fuels_gt_1=>//.
  - iIntros "H". iApply Hccl.
    rewrite /Δ' /= (envs_snoc_sound Δother false i).
    + by iApply "H2".
    + rewrite /Δother. eapply envs_lookup_envs_delete =>//.
Qed.

Lemma tac_wp_pure
  `{LM : LiveModel aneris_lang (joint_model M net_model)} `{!LiveModelEq LM} `{!anerisG LM Σ}
  Δ Δ'other tid E i e1 e2  φ n Φ fs ip :
  envs_lookup i Δ = Some (false, has_fuels tid fs)%I →
  PureExec φ n (mkExpr ip e1) (mkExpr ip e2) →
  (∀ (ρ : usr_role M) (f : nat), fs !! ρ = Some f → (f ≥ n)%nat) ->
  fs ≠ ∅ ->
  φ →
  let Δother := envs_delete true i false Δ in
  MaybeIntoLaterNEnvs n Δother Δ'other →
  let Δ' := envs_snoc Δ'other false i (has_fuels tid ((λ m, m - n)%nat <$> fs)) in
  envs_entails Δ' (WP mkExpr ip e2 @ tid; E {{ Φ }}) →
  envs_entails Δ (WP mkExpr ip e1 @ tid; E {{ Φ }}).
Proof.
  rewrite envs_entails_unseal=> ???.
  ...
  iApply (tac_wp_pure_helper_2 _ _ [] with "[H2] [H1]") =>//.
...
Lemma tac_wp_value
  `{LM : LiveModel aneris_lang (joint_model M net_model)} `{!LiveModelEq LM} `{!anerisG LM Σ}
  Δ tid E (Φ : aneris_val → iPropI Σ) v :
  envs_entails Δ (|={E}=> Φ v) → envs_entails Δ (WP aneris_of_val v @ tid; E {{ Φ }}).
Proof. rewrite envs_entails_unseal=> ->. iIntros "H". iApply wp_value_fupd. destruct v=>//. Qed.
...
Lemma tac_wp_bind
  ...
  envs_entails Δ (WP mkExpr ip e @ s; E {{ v, WP f (aneris_of_val v) @ s; E {{ Φ }} }})%I →
  envs_entails Δ (WP fill K (mkExpr ip e) @ s; E {{ Φ }}).
...
(** Heap tactics *)
Section heap.
Context `{LM : LiveModel aneris_lang (joint_model M net_model)} `{!LiveModelEq LM} `{!anerisG LM Σ}.
Implicit Types P Q : iProp Σ.
Implicit Types Φ : aneris_val → iProp Σ.
Implicit Types Δ : envs (uPredI (iResUR Σ)).
Implicit Types v : aneris_val.
Implicit Types tid : locale aneris_lang.
