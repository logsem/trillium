...
Definition fmlabel (FM :FairModel) : Type :=
  (fmrole FM * fmaction FM) + fmconfig FM.
...
Definition mtrace (M:FairModel) :=
  trace (M.(fmstate)) (fmlabel M).
...
Section model_traces.
  ...
  Definition live_mdl_role (ρ : fmlabel M) (δ : fmstate M) : Prop :=
    match ρ with
    | inl (ρ,_) => role_enabled_model ρ δ
