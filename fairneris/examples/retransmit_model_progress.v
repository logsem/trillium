...
Lemma retransmit_trace_delivers_0 mtr n :
  (∀ n, pred_at mtr n (λ _ l, l ≠ Some $ inr ((), Some mAB))) →
...
Lemma A_always_live (mtr : mtrace) n :
  infinite_trace mtr →
  pred_at mtr n (λ s _, retransmit_role_enabled_model (Arole) s).
Proof.
...
Lemma retransmit_fair_trace_eventually_Ndeliver mtr :
  ...
  ∃ n, pred_at mtr n (λ st l, l = Some $ inr ((),Some mAB)).
Proof.
  intros Hafter Hvalid Hfair.
  assert ((∃ n, pred_at mtr n (λ _ l, l = Some (inr ((), Some mAB)))) ∨
            ¬ ∃ n, pred_at mtr n (λ _ l, l = Some (inr ((), Some mAB))))
    as Hpred_at.
...
Lemma Ndeliver_adds_to_buffer msg s bs (mtr:mtrace) :
  mtrace_valid (s -[inr ((),Some msg)]-> mtr) →
  (trfirst (s -[inr ((),Some msg)]-> mtr)).2 !!! m_destination msg = bs →
...
Proof.
  ...
  assert ((pred_at (s -[ inr ((), Some mAB) ]-> mtr') 0
                   (λ δ _, retransmit_role_enabled_model (Brole) δ)) ∨
            ¬ pred_at (s -[ inr ((), Some mAB) ]-> mtr') 0
                   (λ δ _, retransmit_role_enabled_model (Brole) δ)) as Hrole.
  ...
  { exists (n+m). rewrite pred_at_sum Hmtr'. eapply pred_at_impl; [|done].
    ...
    set_solver. }
  assert (∃ bs, (trfirst (s -[ inr ((), Some mAB) ]-> mtr')).2 !!!
...
Lemma retransmit_fair_traces_terminate_aux mtr :
  (trfirst mtr).1.1 = Received → retransmit_terminating_role (Brole) mtr.
Proof.
  intros Htrfirst.
...
Proof.
  ...
  assert (retransmit_terminating_role (Brole) mtr') as [m Hterminates];
