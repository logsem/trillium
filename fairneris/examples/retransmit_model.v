...
From fairneris.aneris_lang Require Import ast network lang.
...
| B_RecvFail :
  retransmit_trans Start (Brole, Some (Recv None)) Start
| B_RecvSucc msg :
  retransmit_trans Start (Brole, Some (Recv (Some msg))) Received
.
...
Definition retransmit_live_roles (s : retransmit_state) : gset retransmit_role :=
  {[Arole]} ∪
  (match s with Start => {[Brole]} | _ => ∅ end).
