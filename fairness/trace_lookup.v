From fairness Require Import nat_omega trace_len inftraces trace_utils utils.
...
Section After.
  ...
  Proof. 
    ...
    2: { split; [by intros (?&?&?)| ].
         pose proof (trace_has_len tr) as [len LEN].
         intros NTH.
         opose proof * (proj1 (trace_lookup_dom_strong _ _ LEN n)); eauto.
         eapply trace_len_neg in AFTER; eauto. lia_NO' len. }
  ...
  Proof.
    pose proof (trace_has_len tr) as [len LEN]. 
    opose proof * (proj2 (trace_lookup_dom_strong _ _ LEN i)) as X.
    { eapply trace_lookup_dom; eauto.
...
Section TracesMatch.
  ...
  Proof. 
    ...
    opose proof * (traces_match_same_length _ _ _ _ tr1 tr2) as X; eauto. subst x.
    ...
    opose proof * (proj1 (trace_state_lookup_simpl' tr1 n s1)) as ST1; eauto.
    opose proof * (proj1 (trace_state_lookup_simpl' tr2 n s2)) as ST2; eauto.
    simpl in *.
    pose proof (proj2 (state_lookup_after' _ _ _) ST1) as (atr1 & AFTER1 & A1).
    opose proof * (traces_match_after' _ _ _ _ tr1 tr2) as X; eauto.
    ...
    4: done.
    3: { opose proof * (proj1 (trace_lookup_dom_strong _ _ LEN2 n)); eauto.
         opose proof * (proj1 (trace_lookup_dom_eq _ _ LEN1 n)); eauto.
         lia_NO' len. inversion H0. lia. }
    2: { opose proof * (proj1 (trace_lookup_dom_strong _ _ LEN1 n)); eauto.
         opose proof * (proj1 (trace_lookup_dom_eq _ _ LEN2 n)); eauto.
         lia_NO' len. inversion H0. lia. }
