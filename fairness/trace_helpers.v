...
From fairness Require Import trace_len trace_lookup inftraces fairness trace_utils nat_omega.
...
Section FMTraceHelpers.
  ...
  Proof using. 
    ...
    - specialize (H n).
      ...
      opose proof * (proj1 (label_lookup_states tr (n + k))) as [[st' ST'] _]; eauto.
    - apply pred_at_trace_lookup in EN as [? [Tn EN]]. 
      ...
      + apply pred_at_trace_lookup. destruct PROP as [(?&?&?) | STEP]; eauto.
        opose proof * (proj1 (label_lookup_states tr (n + m))) as [[st' ST'] _]; eauto.
  ...
  Section ValidTracesProperties.
    ...
    Proof using VALID.
      ...
      pose proof (trace_has_len tr) as [len LEN].
      assert (NOmega.lt_nat_l (i + d) len) as DOMid.
      { destruct STEP as [(?&?&?) | STEP]. 
        - eapply state_lookup_dom; eauto.
        - apply NOmega.lt_lt_nat with (m := i + d + 1); [lia| ].
          eapply label_lookup_dom; eauto. }
      pose proof (proj2 (state_lookup_dom _ _ LEN (i + d)) DOMid) as [st' IDTH].
      
      opose proof * (steps_keep_state i _ _ (i + d)) as NEXT_EN; eauto. 
